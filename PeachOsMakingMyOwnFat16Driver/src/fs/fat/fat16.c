...
{
    int bootBeginningSector;
    int fatTableBeginningSector;
    int rootDirBeginningSector;
    int dataRegionBeginningSector;
...
{
    return (fp->helperData.dataRegionBeginningSector + (cluster - 2) * fp->headers.primary_header.sectors_per_cluster);
}
...
int fat16_consult_fat_table(struct disk* disk, int clusterToCheck)
{
    ...
    struct fat_private* private_data = disk->fs_private;
    int byteToReadFromFat = (private_data->helperData.fatTableBeginningSector * disk->sector_size) + (2 * clusterToCheck);
    int buffer;
...
int fat16_read(struct disk *disk, void* descriptor, uint32_t sizeOfElement, uint32_t numOfElements, char* out)
{
    ...
    // if(sizeOfElement * numOfElements > disk->sector_size) return -ELAZY; // i dont care right now

    // // less than a sector
    // void* tempOut = kzalloc(1); // allocate 4096 bytes
    // int clusterToReadFrom = fat_desc->file.low_16_bits_first_cluster; 
    // int sectorToReadFrom = clusterToReadFrom - 2; 
    // int byteToReadFrom = sectorToReadFrom * disk->sector_size;
    // if(diskstreamer_read(private_data->cluster_read_stream))
    return 6969;
}
...
int fat16_resolve(struct disk* disk)
{
    ...
    tempDataAccess->bootBeginningSector = 0; 
    
    tempDataAccess->fatTableBeginningSector = tempHeaderAccess->reserved_sectors;
    
    tempDataAccess->rootDirBeginningSector = 
    ...
    tempDataAccess->dataRegionBeginningSector = 
        tempDataAccess->rootDirBeginningSector + (calculation / 512);
